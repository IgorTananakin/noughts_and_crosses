const express = require('express');//express для получения статических файлов. ws сам websocket
...
const opponents = {};//храним связку игроков по id
let clientIdsWaitingMatch = [];

wss.on("connection", connection => {
  //получения каждого клиента
  const clientId = createClientId();
  clientConnections[clientId] = connection;

  matchClients(clientId);//сопоставляем клиентов
  ...
  });//подпишемся на принятие любых сообщений с клиента
